...
const CartPage = () => {
  ...
  if(cartItems.length < 1){
    return (
      <NotFoundWrapper>
        <div className='container'>Nenhum item encontrado no carrinho.</div>
      </NotFoundWrapper>
    )
  }
        ...
        <div className='cart-grid grid'>
          ...
          <div className='cart-grid-left'>
            <div className='flex flex-wrap flex-between'>
              <div className='cart-count-info'>
                <span className='fw-7 fs-18'>{total_items}</span> Curso no Carrinho
              </div>
              <button type = "button" className='cart-clear-btn flex fs-15 fw-6 text' onClick={() => clearCart()}>
                <MdClear className='text-pink' />
                <span className='d-inline-block text-pink'>Limpar tudo</span>
              </button>
            </div>
          ...
          <div className='cart-grid-right'>
            <div className='cart-total'>
              ...
              <button type = "button" className='checkout-btn bg-purple text-white fw-6'>Confira</button>
            </div>
          </div>
